...
import { User, Package, Calendar, Mail, Phone, MapPin, CreditCard, Truck, LogOut, Loader2, ChevronDown, ChevronUp, Eye } from 'lucide-react';
import Header from "@/components/Header";
import Footer from "@/components/Footer";
import { useAuth } from '../hooks/useAuth'; // Import the useAuth hook
import { apiGet } from '../utils/tokenManager'; // Import API utility with automatic token management
...
const ProfilePage = () => {
  ...
  // Use the auth hook for automatic token management
  ...
  useEffect(() => {
    ...
    // If not authenticated, redirect will be handled by useAuth
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (err.message === 'Authentication required' || err.message === 'Authentication failed') {
        // Token was invalid, useAuth will handle redirect
        return;
      }
  ...
  const fetchUserOrders = async () => {
    try {
      const response = await apiGet('api/user/orders/');
  ...
  // If not authenticated, show loading (redirect will be handled by useAuth)
  if (!isAuthenticated) {
    return (
      <div>
        <Header />
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="flex items-center space-x-3 bg-white p-6 rounded-lg shadow-sm">
            <Loader2 className="h-6 w-6 animate-spin text-blue-600" />
            <span className="text-gray-700">Redirection vers la connexion...</span>
          </div>
        </div>
        <Footer />
      </div>
                ...
                <div className="bg-white rounded-lg sm:rounded-xl shadow-sm p-4 sm:p-6 lg:sticky lg:top-28">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    ...
                    <div className="pt-3 sm:pt-4 border-t">
                      <h4 className="font-medium text-gray-900 mb-3 sm:mb-4 text-sm sm:text-base">Statistiques du compte</h4>
                      <div className="space-y-2 sm:space-y-3">
                        <div className="flex justify-between items-center p-2 sm:p-3 bg-blue-50 rounded-lg">
                          <span className="text-xs sm:text-sm text-gray-700">Commandes totales</span>
                          <span className="font-semibold text-blue-700 text-sm sm:text-base">{orders.length}</span>
                        </div>
